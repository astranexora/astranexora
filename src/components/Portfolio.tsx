...
export default function Portfolio() {
  ...
  return (
    <section id="portfolio" className="relative py-32 overflow-hidden bg-cloud">
      {/* Background */}
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="font-display font-bold text-4xl md:text-5xl xl:text-6xl text-ink-900 mb-5">
          ...
          {tags.map(tag => (
            <button
              ...
              className={`px-5 py-2 rounded-full font-display text-xs font-semibold transition-all duration-300 ${
                filter === tag
                  ? 'bg-gradient-to-r from-brand-500/20 to-brand-600/10 text-ink-900 border border-brand-500/30 shadow-[0_0_20px_rgba(0,102,255,0.1)]'
                  : 'glass text-ink-500 border border-ink-200 hover:text-ink-700 hover:border-ink-200'
              }`}
            >
              {tag}
            </button>
        ...
        {featured && (
          ...
          >
            <Link
              ...
              className="group relative block rounded-3xl overflow-hidden border border-ink-200 hover:border-brand-500/20 transition-all duration-500"
              style={{
                background: 'linear-gradient(135deg, rgba(0,102,255,0.06), rgba(0,102,255,0.03))',
              }}
            >
              <div className="grid lg:grid-cols-2 gap-0">
                {/* Image */}
                <div className="relative h-64 lg:h-auto lg:min-h-[400px] overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-brand-500/15 to-brand-600/8" />
                  {featured.image ? (
                    <img
                      src={featured.image}
                      alt={featured.title}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-brand-500/20 to-brand-600/10 flex items-center justify-center">
                        <Sparkles className="w-12 h-12 text-ink-400" />
                      </div>
                    </div>
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r from-cloud via-cloud/50 to-transparent" />

                  {/* Featured badge */}
                  <div className="absolute top-6 left-6 px-4 py-1.5 rounded-full bg-gradient-to-r from-brand-500/20 to-brand-600/10 border border-brand-500/20 backdrop-blur-sm">
                ...
                <div className="p-8 lg:p-12 flex flex-col justify-center relative">
                  ...
                  <h3 className="font-display font-bold text-3xl md:text-4xl text-ink-900 mb-5 group-hover:text-gradient-blue transition-all duration-300 leading-tight">
                    {featured.title}
                  </h3>
                  <p className="text-ink-700 leading-[1.7] mb-8 max-w-md">
                    {featured.shortDesc}
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-7">
                    {featured.results?.slice(0, 4).map((result, i) => (
                      <div key={i} className="glass rounded-xl p-3.5 border border-ink-200">
              ...
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
                style={{ background: 'radial-gradient(ellipse at center, rgba(0,102,255,0.08), transparent 70%)' }} />
            </Link>
          </motion.div>
        )}
        ...
        >
          {[
            ...
            <div key={stat.label} className="glass rounded-2xl p-5 border border-ink-200 text-center">
              <div className="font-display font-bold text-2xl md:text-3xl text-gradient-blue mb-1">{stat.value}</div>
              <div className="text-xs text-ink-500 font-display">{stat.label}</div>
            </div>
          ))}
        </motion.div>
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            background: `radial-gradient(circle 120px at ${mousePos.x}% ${mousePos.y}%, rgba(0,102,255,0.1), transparent)`,
          }}
        />
        ...
        <div className="relative h-48 overflow-hidden">
          ...
          <motion.div
            ...
            className="absolute top-4 right-4 w-9 h-9 rounded-xl glass-blue flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 border border-brand-500/20"
          >
