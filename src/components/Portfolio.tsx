        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          >
            ...
            <h2 className="font-display font-bold text-4xl md:text-5xl xl:text-6xl text-ink-900 tracking-tight leading-[1.05]">
              Crafting Digital
              <br />
              <span className="text-gradient-navy">Masterpieces</span>
            </h2>
          </motion.div>
          <motion.p
          ...
          {tags.map(tag => (
            <button
              ...
              className={`px-5 py-2 rounded-full font-display text-xs font-semibold transition-all duration-300 ${
                filter === tag
                  ? 'bg-brand-900 text-white border border-brand-900 shadow-[0_4px_16px_rgba(15,27,61,0.2)]'
                  : 'glass text-ink-500 border border-ink-200 hover:text-ink-700 hover:border-brand-900/20'
              }`}
            >
              {tag}
            </button>
        ...
        {featured && (
          ...
          >
            <Link
              ...
              className="group relative block rounded-3xl overflow-hidden border border-ink-200 hover:border-brand-900/20 transition-all duration-500 shadow-[0_4px_30px_rgba(10,10,15,0.04)]"
            >
              <div className="grid lg:grid-cols-12 gap-0">
                {/* Image — 7 cols */}
                <div className="relative h-72 lg:h-auto lg:min-h-[440px] lg:col-span-7 overflow-hidden navy-duotone">
                  <div className="absolute inset-0 bg-gradient-to-br from-brand-900/8 to-brand-950/4" />
                  {featured.image ? (
                    <img
                      src={featured.image}
                      alt={featured.title}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-brand-900/15 to-brand-950/8 flex items-center justify-center">
                        <Sparkles className="w-12 h-12 text-brand-900/40" />
                      </div>
                    </div>
                  )}
                  ...
                  {/* Featured badge */}
                  <div className="absolute top-6 left-6 px-4 py-1.5 rounded-full bg-brand-900 border border-brand-400/30 shadow-lg">
                ...
                <div className="lg:col-span-5 p-8 lg:p-10 flex flex-col justify-center relative bg-white">
                  <div className="mb-2 text-sm text-brand-900 font-display font-medium">{featured.tag}</div>
                  <h3 className="font-display font-bold text-2xl md:text-3xl text-ink-900 mb-5 group-hover:text-brand-900 transition-all duration-300 leading-tight">
                    {featured.title}
                  </h3>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-7">
                    {featured.results?.slice(0, 4).map((result, i) => (
                      <div key={i} className="glass-blue rounded-xl p-3.5 border border-brand-900/10">
                        <div className="text-xl font-display font-bold text-gradient-navy">{result.value}</div>
                  ...
                  {/* CTA */}
                  <div className="flex items-center gap-2 text-brand-900 font-display font-semibold text-sm group-hover:gap-4 transition-all duration-300">
                    View Case Study
              ...
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
                style={{ background: 'radial-gradient(ellipse at center, rgba(15,27,61,0.06), transparent 70%)' }} />
            </Link>
          </motion.div>
        )}
  ...
  return (
    ...
    >
      <Link
        ...
        className="group relative block glass rounded-2xl overflow-hidden border border-ink-200 hover:border-brand-900/20 transition-all duration-500"
        style={{ transformStyle: 'preserve-3d', willChange: 'transform' }}
      >
        {/* Mouse-follow glow */}
        <div
          className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
          style={{ background: `radial-gradient(circle 120px at ${mousePos.x}% ${mousePos.y}%, rgba(15,27,61,0.1), transparent)` }}
        />

        {/* Image area */}
        <div className="relative h-48 overflow-hidden navy-duotone">
          <div
          ...
          {/* Arrow */}
          <div className="absolute top-4 right-4 w-9 h-9 rounded-xl glass-blue flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 border border-brand-900/20">
        ...
        <div className="p-6 relative">
          <h3 className="font-display font-semibold text-ink-900 text-base mb-2.5 group-hover:text-brand-900 transition-colors duration-300 leading-snug">
            {project.title}
          </h3>
