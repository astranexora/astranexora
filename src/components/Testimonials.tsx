...
export default function Testimonials() {
  ...
  return (
    <section id="testimonials" className="relative py-28 md:py-36 overflow-hidden bg-cloud">
      ...
      <div ref={ref} className="max-w-7xl mx-auto px-6">
        ...
        <div className={`max-w-4xl mx-auto mb-10 transition-all duration-800 delay-200 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
          <div className="relative glass-blue rounded-3xl p-8 md:p-12 border border-brand-500/15 overflow-hidden shadow-[0_0_60px_rgba(0,102,255,0.1)]">
            {/* BG shimmer */}
            ...
            <div className="absolute top-8 right-10 opacity-[0.06]">
              <Quote className="w-28 h-28 text-brand-600" />
            </div>
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-brand-500 to-brand-600 flex items-center justify-center font-display font-bold text-white text-sm shadow-[0_0_16px_rgba(0,102,255,0.3)]">
                    {t.avatar}
                  </div>
                  <div>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={prev}
                    className="w-9 h-9 rounded-lg glass-blue flex items-center justify-center text-ink-500 hover:text-ink-900 hover:border-brand-500/20 transition-all duration-200"
                  >
          ...
          {testimonials.map((_, i) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-400 ${
                active === i ? 'w-8 bg-electric shadow-[0_0_8px_rgba(0,207,255,0.6)]' : 'w-1.5 bg-ink-200 hover:bg-ink-300'
              }`}
            />
          ))}
        </div>
          ...
          {testimonials.map((t, i) => (
            <button
              ...
              className={`flex items-center gap-3 px-4 py-2 rounded-xl border transition-all duration-300 ${
                active === i
                  ? 'border-brand-500/30 bg-brand-500/8 shadow-[0_0_12px_rgba(0,102,255,0.1)]'
                  : 'border-ink-200 bg-mist hover:border-ink-200'
              }`}
            >
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold transition-all duration-300 ${
                active === i
                  ? 'bg-gradient-to-br from-brand-500 to-brand-600 text-white shadow-[0_0_10px_rgba(0,102,255,0.3)]'
