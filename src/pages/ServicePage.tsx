...
function WebsiteDevVisual() {
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      {/* Browser window */}
      <div className="w-full max-w-sm bg-white rounded-2xl border border-brand-500/20 shadow-xl shadow-brand-500/5 overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          <div className="h-24 rounded-xl bg-gradient-to-br from-brand-500/15 to-brand-600/10 border border-brand-500/20 flex items-center justify-center">
            <div className="text-center">
              <div className="w-8 h-1 bg-brand-500/50 rounded mx-auto mb-2" />
      ...
      <div className="absolute -bottom-2 -right-2 bg-brand-500/90 backdrop-blur-xl rounded-xl px-4 py-2.5 border border-brand-500/30 shadow-lg">
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      <div className="w-full max-w-sm space-y-3">
        {/* Ranking chart */}
        <div className="bg-white rounded-2xl border border-brand-500/20 p-4 shadow-sm">
          ...
          <div className="flex items-end gap-1.5 h-20">
            {[30, 55, 45, 70, 60, 85, 80, 95].map((h, i) => (
              <div key={i} className="flex-1 rounded-t-sm" style={{ height: `${h}%`, background: i > 4 ? 'linear-gradient(to top, #0066FF, #60a5fa)' : 'rgba(0,102,255,0.15)' }} />
            ))}
          </div>
        </div>
        {/* Search results mockup */}
        {['#1 Brand Strategy Guide', '#2 Digital Marketing Tips', '#3 SEO Best Practices'].map((item, i) => (
          <div key={i} className={`rounded-xl px-4 py-3 flex items-center gap-3 border transition-all ${i === 0 ? 'bg-brand-500/10 border-brand-500/30' : 'bg-mist border-ink-200'}`}>
            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 ? 'bg-brand-500 text-white' : 'bg-ink-100 text-ink-400'}`}>{i + 1}</div>
      ...
      <div className="w-full max-w-sm space-y-3">
        <div className="bg-white rounded-2xl border border-brand-500/20 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-brand-500">This Month</span>
          </div>
          <div className="grid grid-cols-3 gap-3 mb-4">
            {[{ icon: Heart, label: 'Likes', val: '12.4K', c: 'text-pink-400' }, { icon: MessageCircle, label: 'Comments', val: '1.8K', c: 'text-brand-500' }, { icon: Eye, label: 'Reach', val: '84K', c: 'text-purple-400' }].map(({ icon: Icon, label, val, c }) => (
          ...
          <div className="h-1.5 bg-ink-100 rounded-full overflow-hidden">
            <div className="h-full w-4/5 rounded-full bg-gradient-to-r from-pink-500 to-brand-500" />
          </div>
          <div className="flex justify-between mt-1">
            <span className="text-xs text-ink-300">Goal Progress</span>
            <span className="text-xs text-brand-500">80%</span>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {[{ name: 'Instagram', col: 'from-pink-500/15 to-orange-500/15', b: 'border-pink-500/20' }, { name: 'LinkedIn', col: 'from-brand-500/15 to-brand-600/15', b: 'border-brand-500/20' }].map(({ name, col, b }) => (
...
function BrandingVisual() {
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      <div className="w-full max-w-sm space-y-3">
        <div className="bg-white rounded-2xl border border-brand-500/20 p-5 shadow-sm">
          <div className="text-xs text-ink-400 mb-3 uppercase tracking-widest">Brand Identity System</div>
          {/* Logo area */}
          <div className="h-20 rounded-xl bg-gradient-to-br from-brand-500/10 to-brand-600/5 border border-brand-500/20 flex items-center justify-center mb-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          {['Business Card', 'Letterhead', 'Favicon'].map(item => (
            <div key={item} className="rounded-xl bg-mist border border-ink-200 p-2 text-center">
              <div className="w-6 h-6 rounded-lg bg-brand-500/10 mx-auto mb-1 flex items-center justify-center">
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      <div className="w-full max-w-sm space-y-3">
        <div className="bg-white rounded-2xl border border-brand-500/20 p-4 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-1 mb-3">
            ...
            {Array.from({ length: 28 }, (_, i) => (
              <div key={i} className={`rounded h-6 flex items-center justify-center text-xs ${[2,5,9,13,16,20,23].includes(i) ? 'bg-brand-500/30 text-brand-600 font-bold' : 'bg-mist text-ink-300'}`}>
                {i + 1}
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {[{ icon: FileText, label: 'Blog Posts', val: '8/mo', c: 'text-brand-500' }, { icon: Play, label: 'Videos', val: '4/mo', c: 'text-red-400' }, { icon: PenTool, label: 'Graphics', val: '12/mo', c: 'text-purple-400' }, { icon: Mail, label: 'Newsletters', val: '4/mo', c: 'text-green-600' }].map(({ icon: Icon, label, val, c }) => (
    ...
    <div className="relative w-full h-full flex items-center justify-center">
      <div className="w-full max-w-sm space-y-3">
        <div className="bg-white rounded-2xl border border-brand-500/20 p-4 shadow-sm">
          ...
          {[{ label: 'Open Rate', val: 47, color: 'from-brand-500 to-cyan-400' }, { label: 'Click Rate', val: 12, color: 'from-brand-600 to-brand-500' }, { label: 'Conversion', val: 6, color: 'from-green-500 to-emerald-400' }].map(({ label, val, color }) => (
        ...
        <div className="bg-white rounded-2xl border border-ink-200 p-4 shadow-sm">
          ...
          <div className="inline-block bg-brand-500 text-white text-xs px-4 py-1.5 rounded-full">Open Now →</div>
        </div>
      </div>
    </div>
  );
...
function GenericVisual({ icon: Icon, gradient }: { icon: React.ElementType; gradient: string }) {
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      <div className="w-full max-w-sm space-y-3">
        <div className="bg-white rounded-2xl border border-brand-500/20 p-6 flex items-center justify-center shadow-sm">
...
const serviceVisuals: Record<string, React.FC> = {
  ...
  'wordpress-development': () => <GenericVisual icon={Globe} gradient="from-brand-500 to-brand-600" />,
  'wix-development': () => <GenericVisual icon={Layout} gradient="from-cyan-500 to-brand-500" />,
  'website-revamp': () => <GenericVisual icon={RefreshCw} gradient="from-orange-500 to-brand-500" />,
  ...
  'personal-branding': () => <GenericVisual icon={User} gradient="from-brand-500 to-brand-600" />,
  'presentation-design': () => <GenericVisual icon={Presentation} gradient="from-brand-500 to-cyan-400" />,
...
const serviceMetrics: Record<string, { label: string; val: string; sub: string; icon: React.ElementType; color: string }[]> = {
  'website-development': [
    { label: 'PageSpeed Score', val: '95+', sub: 'avg. after launch', icon: Activity, color: 'text-brand-500' },
  ...
  'seo-optimization': [
    ...
    { label: 'Keyword Rankings', val: 'Top 3', sub: 'targeted terms', icon: Search, color: 'text-brand-500' },
  ...
  'social-media-marketing': [
    ...
    { label: 'Brand Reach', val: '+5.6x', sub: 'vs. baseline', icon: Eye, color: 'text-brand-500' },
    { label: 'Social Revenue', val: '+180%', sub: 'attributed sales', icon: DollarSign, color: 'text-green-600' },
  ],
  'email-marketing': [
    { label: 'Open Rates', val: '42%+', sub: 'vs. 21% industry avg', icon: Mail, color: 'text-brand-500' },
  ...
  'brand-identity-design': [
    ...
    { label: 'Trust Score', val: '9.2/10', sub: 'customer surveys', icon: Shield, color: 'text-brand-500' },
...
const defaultMetrics = [
  ...
  { label: 'Projects Delivered', val: '100+', sub: 'and counting', icon: Rocket, color: 'text-brand-500' },
...
export default function ServicePage() {
  ...
  const HeroVisual = serviceVisuals[service.id] || (() => <GenericVisual icon={Icon} gradient="from-brand-500 to-cyan-500" />);
  ...
  return (
    <div className="min-h-screen bg-cloud text-ink-900 overflow-x-hidden">
      ...
      <section className="relative min-h-[90vh] flex items-center overflow-hidden pt-20">
        ...
        <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(rgba(0,102,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(0,102,255,0.03) 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
        <div className="absolute inset-0 aurora-bg opacity-30" />

        {/* Glow orbs */}
        <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full opacity-10 blur-3xl animate-float" style={{ background: 'radial-gradient(circle, #0066FF, transparent)' }} />
        <div className="absolute bottom-1/4 right-1/3 w-64 h-64 rounded-full opacity-8 blur-3xl" style={{ background: 'radial-gradient(circle, #06b6d4, transparent)', animation: 'float 6s ease-in-out infinite reverse' }} />
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(20)].map((_, i) => (
            <div key={i} className="absolute w-1 h-1 rounded-full bg-brand-500/20 animate-float"
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div>
              ...
              <div className="inline-flex items-center gap-3 mb-8">
                <div className="w-14 h-14 rounded-2xl bg-brand-500/10 border border-brand-500/20 flex items-center justify-center shadow-lg shadow-brand-500/10">
                  <Icon className="w-7 h-7 text-brand-500" />
                </div>
                <div className="h-px w-12 bg-gradient-to-r from-brand-500 to-transparent" />
                <span className="text-xs text-brand-500 uppercase tracking-widest font-semibold">Service</span>
              </div>
            ...
            <div className="relative hidden lg:block">
              <div className="relative bg-white/60 backdrop-blur-xl rounded-3xl p-8 border border-brand-500/20 shadow-2xl shadow-brand-500/5" style={{ minHeight: 380 }}>
                {/* Glow border effect */}
                <div className="absolute inset-0 rounded-3xl opacity-10 blur-xl" style={{ background: 'linear-gradient(135deg, #0066FF, transparent, #06b6d4)' }} />
                <HeroVisual />
              </div>
              {/* Corner accents */}
              <div className="absolute -top-3 -right-3 w-12 h-12 border-t-2 border-r-2 border-brand-500/30 rounded-tr-2xl" />
              <div className="absolute -bottom-3 -left-3 w-12 h-12 border-b-2 border-l-2 border-brand-500/30 rounded-bl-2xl" />
            </div>
          </div>
        </div>
        ...
        <div ref={deliverSection.ref} className="relative z-10 max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {service.features.map((feature, i) => (
              <div
                key={feature}
                className={`group relative glass-blue rounded-2xl p-6 border border-brand-500/15 hover:border-brand-500/40 hover:-translate-y-1 transition-all duration-300 cursor-default ${deliverSection.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${i * 80}ms` }}
              >
                <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ background: 'linear-gradient(135deg, rgba(0,102,255,0.04), transparent)' }} />
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-12">
            {service.benefits.map((b, i) => (
              <div
                key={b.title}
                className={`group glass rounded-2xl p-7 border border-ink-200 hover:border-brand-500/30 hover:-translate-y-1 transition-all duration-300 ${deliverSection.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${(service.features.length + i) * 80}ms` }}
              >
                <div className="w-10 h-10 rounded-xl bg-brand-500/8 flex items-center justify-center mb-4 group-hover:bg-brand-500/10 transition-colors">
            ...
            {service.process.map((step, i) => (
              ...
              >
                ...
                {i < service.process.length - 1 && (
                  <div className="hidden lg:block absolute top-10 left-full w-full h-px z-0" style={{ background: 'linear-gradient(90deg, rgba(0,102,255,0.3), transparent)', width: 'calc(100% - 2rem)', left: 'calc(100% - 1rem)' }} />
                )}
                <div className="glass-blue rounded-2xl p-7 border border-brand-500/15 hover:border-brand-500/40 hover:-translate-y-1 transition-all duration-300 h-full relative z-10 overflow-hidden">
                  <div className="absolute top-0 right-0 text-6xl font-black text-brand-500/5 leading-none select-none pr-3 pt-1">{step.step}</div>
                  {/* Step number badge */}
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-brand-500 to-brand-600 flex items-center justify-center mb-5 shadow-lg shadow-brand-500/10">
        ...
        <div ref={whySection.ref} className="relative z-10 max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-5">
            {whyUs.map(({ icon: WIcon, title, desc }, i) => (
              <div
                key={title}
                className={`group glass-blue rounded-2xl p-6 border border-brand-500/15 hover:border-brand-500/40 hover:-translate-y-2 transition-all duration-300 text-center ${whySection.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${i * 80}ms` }}
              >
                <div className="w-12 h-12 rounded-2xl bg-brand-500/10 border border-brand-500/20 flex items-center justify-center mx-auto mb-4 group-hover:bg-brand-500/15 group-hover:scale-110 transition-all duration-300">
                  <WIcon className="w-6 h-6 text-brand-500" />
                </div>
        ...
        <div ref={metricsSection.ref} className="relative z-10 max-w-7xl mx-auto px-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {metrics.map(({ label, val, sub, icon: MIcon, color }, i) => (
              <div
                key={label}
                className={`group relative glass rounded-2xl p-8 border border-ink-200 hover:border-brand-500/30 hover:-translate-y-1 transition-all duration-300 overflow-hidden ${metricsSection.visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${i * 100}ms` }}
              >
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300" style={{ background: 'linear-gradient(135deg, rgba(0,102,255,0.03), transparent)' }} />
          ...
          <div className="max-w-3xl mx-auto">
            <h3 className="font-display font-bold text-2xl text-ink-900 text-center mb-8">Common <span className="text-gradient-blue">Questions</span></h3>
            <div className="space-y-3">
              {service.faqs.map((faq, i) => (
                <details key={i} className="group glass rounded-2xl overflow-hidden border border-ink-200 hover:border-brand-500/30 transition-all duration-300">
                  <summary className="p-5 cursor-pointer flex items-center justify-between text-ink-700 hover:text-ink-900 font-display font-medium text-sm list-none">
                    {faq.q}
                    <ChevronRight className="w-4 h-4 text-brand-500 flex-shrink-0 group-open:rotate-90 transition-transform" />
                  </summary>
      ...
      <section className="py-32 relative overflow-hidden">
        ...
        <div className="absolute inset-0" style={{ backgroundImage: 'linear-gradient(rgba(0,102,255,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(0,102,255,0.03) 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] opacity-10 blur-3xl" style={{ background: 'radial-gradient(ellipse, #0066FF, transparent)' }} />

        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center">
          <div className="inline-flex w-16 h-16 rounded-2xl bg-brand-500/10 border border-brand-500/20 items-center justify-center mb-8 mx-auto shadow-lg shadow-brand-500/10">
