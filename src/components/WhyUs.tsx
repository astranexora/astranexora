...
const reasons = [
  ...
  },
  {
    ...
    gradient: 'from-brand-900 to-brand-500',
    glow: 'rgba(59,74,120,0.4)',
  },
  {
    icon: Shield,
...
function ReasonCard({ reason, index, visible }: { reason: typeof reasons[0]; index: number; visible: boolean }) {
  ...
  return (
    ...
    >
      {/* Hover glow bg */}
      <div className="absolute inset-0 bg-gradient-to-br from-brand-900/0 to-brand-900/0 group-hover:from-brand-900/6 group-hover:to-brand-900/4 transition-all duration-500 rounded-2xl" />

      {/* Subtle navy glow on hover */}
      <div
        className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
        style={{ boxShadow: '0 0 40px rgba(15,27,61,0.12), 0 0 80px rgba(15,27,61,0.06)' }}
      />
      ...
      {/* Top border sweep */}
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-brand-900 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

      {/* Bottom border sweep */}
      <div className="absolute bottom-0 left-0 h-px bg-gradient-to-r from-transparent via-brand-900 to-transparent w-0 group-hover:w-full transition-all duration-700" />

      {/* Editorial number */}
      <div className="absolute top-5 right-6 editorial-num font-display font-black text-3xl leading-none text-brand-900/[0.08] group-hover:text-brand-900/[0.14] transition-colors duration-500 select-none">
        {String(index + 1).padStart(2, '0')}
      </div>

      <div className="relative z-10">
        {/* Icon */}
        <div className="w-12 h-12 rounded-full bg-brand-900 flex items-center justify-center mb-6 group-hover:scale-110 group-hover:shadow-[0_0_20px_rgba(15,27,61,0.3)] transition-all duration-300">
          <reason.icon className="w-6 h-6 text-white" />
        </div>

        {/* Title */}
        <h3 className="font-display font-semibold text-ink-900 text-lg mb-3 group-hover:text-brand-900 transition-colors duration-300 leading-tight">
          {reason.title}
        </h3>
